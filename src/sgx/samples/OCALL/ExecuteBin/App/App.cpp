...
int main(int argc, char const *argv[]) {
	...
	FILE* file = fopen("./execute", "r");
	void* ptr = static_cast<void*>(file);

	sgx_status_t status = executeBinEnclave(global_eid, ptr);
	if (status != SGX_SUCCESS) {
		std::cout << "FAILURE!!!" << std::endl;
	}
	printf("Successful using ExecuteBin function.\n");
